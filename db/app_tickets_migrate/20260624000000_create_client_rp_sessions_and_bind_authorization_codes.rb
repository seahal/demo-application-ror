...
class CreateClientRpSessionsAndBindAuthorizationCodes < ActiveRecord::Migration[8.2]
  disable_ddl_transaction!

  def change
    create_table :client_rp_sessions do |t|
    ...
    add_index :client_rp_sessions, :public_id, unique: true
    add_index :client_rp_sessions,
              %i[client_token_id oidc_client_id],
              unique: true,
              where: "revoked_at IS NULL",
              name: "idx_active_client_rp_session_per_rp"
    add_index :client_rp_sessions, :refresh_token_digest, unique: true, where: "refresh_token_digest IS NOT NULL"
